...
def ICADataset(Dataset):
    
    def __init__(self,data_loc,pars_list,negatives,logged,threshold = 1e-11,
                 scale_bool = True, comps = 1,ICA_loc="scalers/PCA_flux.bin",
                 comp_loc="scalers/comp_flux.bin",
                 spec_scal_loc="scalers/spec_flux.bin",
                 loads = 0,load_size = 1e6):
        data_table = pd.read_csv(data_loc)
        self.csv = data_table
        self.threshold = threshold
        self.data_load_size = load_size
        self.ICA_loc = ICA_loc
        self.comp_loc = comp_loc
        self.spec_scal_loc = spec_scal_loc
        self.scale_bool = scale_bool
        if scale_bool == False:
            self.spec_scaler = load(self.spec_scal_loc)
            self.ica = load(self.ICA_loc)
            self.ICA_scaler = load(self.comp_loc)
        self.comps = comps
        self.locations = data_table.iloc[:,-1]
        self.pars = data_table.iloc[:,pars_list].to_numpy()
        self.pars_list = pars_list
    ...
    def ICA(self,data,comp = 1):
        if self.scale_bool == True:
            print(f"Attempting n_comp = {comp}")
            self.ica = ICA(n_components = comp,whiten_solver="eigh")
            self.ica.fit(data)
            data = self.ica.transform(data)
            dump(self.ica,self.ICA_loc)
        else:
            data = self.ica.transform(data)
    ...
    def component_scaler(self,data):
        if self.scale_bool == True:
            self.ICA_scaler = StandardScaler()
            data = self.ICA_scaler.fit_transform(data)
            dump(self.ICA_scaler,self.comp_loc)
            self.scale_bool = False
        else:
            data = self.ICA_scaler.transform(data)
        return data
