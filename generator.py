...
def parameter_gen():
    height = np.linspace(1.3,1e4,int((1e4 - 1.3)/0.1))
    spin = np.linspace(0,0.998,int(0.998/0.001))
    inc = np.linspace(1,80,int(79/1e-2))
    rin = np.linspace(-400,-1,int(399/1e-2))
    rout = np.linspace(400,1e5,int((1e5 - 400)))
    z = np.linspace(0,10,int(10/0.001))
    Gamma = np.linspace(1.4,3.4,int(2/0.01))
    Dkpc = np.logspace(np.log10(0.2),10,num=10000)
    Afe = np.linspace(0.5,10,int(9.5/1e-2))
    logNe = np.linspace(15,20,int(5/0.01))
    nH = np.logspace(-10,6,num=10000)
    boost = np.linspace(1e-2,10,int(10/1e-2))
    mass = np.logspace(0,11,num=10000)
    honr = np.linspace(0,0.176,int(0.176/1e-4))
    b1 = np.linspace(0,2,int(2/1e-2))
    b2 = np.linspace(-4,4,int(8/1e-2))
    ...
    grid = np.asarray([height,spin,inc,rin,rout,z,Gamma,Dkpc,Afe,logNe,nH,boost,
                       mass,honr,b1,b2,fmin,fmax,phiAB,g,Anorm],dtype=object)
